...
def forward(m,n,parameter,d_h):    
    ...
    return parameter
def backword(m,n,parameter,lamb): 
    for i in range(m-1,-1,-1):
        for j in range(n-1,-1,-1):
            ...
            #参数导数的叠加
            #l2 regularization
            parameter['dw_r']+=parameter['delta_r'][i][j].dot(q.T)+lamb*parameter['w_r']
            parameter['dw_z']+=parameter['delta_z'][i][j].dot(q.T)+lamb*parameter['w_z']
            
            parameter['db_r']+=parameter['delta_r'][i][j]  +lamb*parameter['b_r']         
            parameter['db_z']+=parameter['delta_z'][i][j]+lamb*parameter['b_z']

    ##        
            parameter['dU']+=parameter['delta_pie'][i][j].dot((r*h_con).T)  +lamb*parameter['U']
            parameter['dw']+=parameter['delta_pie'][i][j].dot(s.T)+parameter['e'][i][j].dot(s.T)+lamb*parameter['w']
            parameter['db']+=parameter['delta_pie'][i][j]+lamb*parameter['b']

            parameter['dw_m']+=parameter['e'][i][j].dot(((parameter['z'][i][j]*h_con).T))+lamb*parameter['w_m']
            parameter['dU_m']+=parameter['e'][i][j]*parameter['h_pie'][i][j].dot((1-parameter['z'][i][j]).T)+lamb*parameter['U_m']
...
def minbatch(parameter,sminus,splus,d_h,d_s,batch,rho,maskval,maxiter,filename,lamb):
    loop=0
    reset=0
    ...
    while 1:
        ...
        for index in range(len(sminus)):
            ...
            #后向计算        
            parameter=backword(m,n,parameter,lamb)   
...
#                print(1)
                ...
                #mask                    
                parameter['w_r']-=ave_dw_r*(rho[0]/np.sqrt(ada_dw_r + 1e-6))*mask[0]
                parameter['w_z']-=ave_dw_z*(rho[1]/np.sqrt(ada_dw_z + 1e-6))*mask[1]
                parameter['b_r']-=ave_db_r*(rho[2]/np.sqrt(ada_db_r + 1e-6))*mask[2]
                parameter['b_z']-=ave_db_z*(rho[3]/np.sqrt(ada_db_z + 1e-6))*mask[3]

                parameter['U']-=ave_dU*(rho[4]/np.sqrt(ada_dU + 1e-6))*mask[4]
                parameter['w']-=ave_dw*(rho[5]/np.sqrt(ada_dw + 1e-6))*mask[5]
                parameter['b']-=ave_db*(rho[6]/np.sqrt(ada_db + 1e-6))*mask[6]
                parameter['w_m']-=ave_dw_m*(rho[7]/np.sqrt(ada_dw_m + 1e-6))*mask[7]
                parameter['U_m']-=ave_dU_m*(rho[8]/np.sqrt(ada_dU_m + 1e-6))*mask[8]

                parameter['w_s']-=ave_dw_s*(rho[9]/np.sqrt(ada_dw_s + 1e-6))*mask[9]
        ...
        print('Loss: '+str(1+result_plus-result_minus))
        if 1+result_plus-result_minus<0.05:
            with open(filename+'_loop'+str(loop)+'_loss'+str(1+result_plus-result_minus)+'.txt','wb') as f:   
                pickle.dump(parameter,f)
                print('number '+str(loop)+' done')
        parameter['result'].append(1+result_plus-result_minus)
    return parameter
...
with open(u'C:\\Users\\Lab\\Desktop\\cross\cross validate1\\S_reduce.txt','rb') as f:   
    sminus_origin=pickle.load(f)
with open(u'C:\\Users\\Lab\\Desktop\\cross\cross validate1\\S_plus.txt','rb') as f:   
...
rho=make_rho()*1000
filename='validate1_v16_dh5_ds1_p1000'
start=clock()
parameter=minbatch(parameter,sminus,splus,d_h,d_s,10,rho,0,200,filename,0.5)
end=clock()
